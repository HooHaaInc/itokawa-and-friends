...
void display(){
    ...
    //Pedimos memoria para la matriz tridimensional de dibujo
	...
	for(int i=0; i<N; ++i){
		proyectar(m, a, triangles[i],plano);
	}
...
void calc_viewport(float *amin, float *bmin, float *amax, float *bmax,int plano){
	...
	for (int i = 0; i < N; ++i)
	{
		...
		if(triangles[i].p3[a] < *xmin) *xmin = triangles[i].p3[j];
		if(triangles[i].p3[b] < *ymin) *ymin = triangles[i].p3[k];
		if(triangles[i].p3[a] > *xmax) *xmax = triangles[i].p3[j];
		if(triangles[i].p3[b] > *ymax) *ymax = triangles[i].p3[k];
	}
}
...
void calc_mat(float m[16],int plano){
	...
	float da = (width-(w*sa))/2-1, db = -((h*sb)-height)/2-1;
...
void proyectar(float m[16], float a[], struct Trianglew t, int plano){
